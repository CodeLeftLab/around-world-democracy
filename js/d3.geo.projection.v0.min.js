! function() {
        ...
        return n.invert = function(t, n) {
            ...
            if (r > va) {
                var e = Math.atan2(n, t),
                    o = a * Math.round((e - va) / a) + va,
                    i = e > o ? -1 : 1,
                    h = r * Math.cos(o - e),
                    u = 1 / Math.tan(i * Math.acos((h - fa) / Math.sqrt(fa * (fa - 2 * h) + r * r)));
    ...
    function z(t, n, a, r, e, o) {
        ...
        return Math.abs(i) > sa ? [i, Math.atan2(r * Math.sin(o), n * e - a * r * M)] : [0, 0]
    }
    ...
    function F(t) {
        return t - 2 * fa * Math.floor((t + fa) / (2 * fa))
    }
    ...
    function C(t, n) {
        var a = Math.sqrt(3);
        return [a * t * (2 * Math.cos(2 * n / 3) - 1) / la, a * la * Math.sin(n / 3)]
    }
    ...
    function O(t, n) {
        var a = Math.sqrt(8 / (3 * fa));
        return [a * t * (1 - Math.abs(n) / fa), a * n]
    }

    function H(t, n) {
        var a = Math.sqrt(4 - 3 * Math.sin(Math.abs(n)));
        return [2 / Math.sqrt(6 * fa) * t * a, i(n) * Math.sqrt(2 * fa / 3) * (2 - a)]
    }

    function I(t, n) {
        var a = Math.sqrt(fa * (4 + fa));
        return [2 / a * t * (1 + Math.sqrt(1 - 4 * n * n / (fa * fa))), 4 / a * n]
    }

    function J(t, n) {
        var a = (2 + va) * Math.sin(n);
        n /= 2;
        for (var r = 0, e = 1 / 0; 10 > r && Math.abs(e) > sa; r++) {
            var o = Math.cos(n);
            n -= e = (n + Math.sin(n) * (o + 2) - a) / (2 * o * (1 + o))
        }
        return [2 / Math.sqrt(fa * (4 + fa)) * t * (1 + Math.cos(n)), 2 * Math.sqrt(fa / (4 + fa)) * Math.sin(n)]
    }

    function K(t, n) {
        return [t * (1 + Math.cos(n)) / Math.sqrt(2 + fa), 2 * n / Math.sqrt(2 + fa)]
    }

    function N(t, n) {
        for (var a = (1 + va) * Math.sin(n), r = 0, e = 1 / 0; 10 > r && Math.abs(e) > sa; r++) n -= e = (n + Math.sin(n) - a) / (1 + Math.cos(n));
        return a = Math.sqrt(2 + fa), [t * (1 + Math.cos(n)) / a, 2 * n / a]
    }
    ...
    function U(t, n) {
        ...
        return [Qa * (h * (u - 1 / u) - 2 * Math.log(u)), Qa * (h * i * (u + 1 / u) - 2 * Math.atan(i))]
    }

    function V(t, n) {
        var a = Math.tan(n / 2);
        return [t * Ra * M(1 - a * a), (1 + Ra) * a]
    }

    function W(t, n) {
        var a = n / 2,
            r = Math.cos(a);
        return [2 * t / la * Math.cos(n) * r * r, la * Math.tan(a)]
    }
    ...
    function X(t, n, a, r, e, o, i, h) {
        ...
        return arguments.length < 8 && (h = 0), u.invert = function(u, s) {
            ...
            } while ((Math.abs(c) > sa || Math.abs(f) > sa) && --g > 0);
            return [v, l]
        }, u
    }
    ...
    function Z(t) {
        ...
        function a(n) {
            ...
            return t.invert && (a.invert = n ? function(n, a) {
                ...
                var c = t.invert(e, o);
                return i || (c[0] += e > 0 ? fa : -fa), c
            } : function(n, a) {
                var e = n > 0 ? -.5 : .5,
                    o = t.invert(n + e * r, a),
                    i = o[0] - e * fa;
                return -fa > i ? i += 2 * fa : i > fa && (i -= 2 * fa), o[0] = i, o
            }), a
        }
        var r = t(va, 0)[0] - t(-va, 0)[0];
        return n.raw = a, n
    }
    ...
    function $(t, n) {
        var a = i(t),
            ...
            u = Math.sin(t) * e,
            M = Math.sin(r * n);
        t = Math.abs(Math.atan2(u, M)), n = h(o), Math.abs(t - va) > sa && (t %= va);
        var s = tn(t > fa / 4 ? va - t : t, n);
        return t > fa / 4 && (M = s[0], s[0] = -s[1], s[1] = -M), s[0] *= a, s[1] *= -r, s
    }
    ...
    function tn(t, n) {
        ...
        var b = Math.cos(n),
            ...
            x = -2 * p * S,
            E = 4 * t / fa;
        if (t > .222 * fa || fa / 4 > n && t > .175 * fa) {
            var P = (d + r * M(l * (1 + e) - d * d)) / (1 + e);
            if (t > fa / 4) return [P, P];
            var k = P,
                _ = .5 * P,
                z = 50;
            P = .5 * (_ + k);
            do {
                var B = Math.sqrt(l - P * P),
                    F = P * (x + R * B) + T * h(P / g) - E;
                if (!F) break;
                0 > F ? _ = P : k = P, P = .5 * (_ + k)
            } while (Math.abs(k - _) > sa && --z > 0)
        } else {
            var A, P = sa,
                z = 25;
            do {
                var G = P * P,
                    ...
                    C = j + (T - R * G) / B;
                P -= A = B ? F / C : 0
            } while (Math.abs(A) > sa && --z > 0)
        }
        return [P, -d - r * M(l - P * P)]
    }
    ...
    function nn(t, n) {
        for (var a = 0, r = 1, e = .5, o = 50;;) {
            var i = e * e,
                ...
                g = Math.sqrt(l),
                d = n + v + e * g;
            if (Math.abs(r - a) < ca || 0 === --o || 0 === d) break;
            d > 0 ? a = e : r = e, e = .5 * (a + r)
        }
        if (!o) return null;
        var b = Math.asin(h),
        ...
        return [fa / 4 * (t * (R + T * g) + x * Math.asin(t / Math.sqrt(f))), b]
    }
    ...
    function rn(t, n) {
        var a, r, e, o, i;
        if (sa > n) return o = Math.sin(t), r = Math.cos(t), a = .25 * n * (t - o * r), [o - a * r, r + a * o, 1 - .5 * n * o * o, t - a];
        if (n >= 1 - sa) return a = .25 * (1 - n), r = b(t), o = g(t), e = 1 / r, i = r * d(t), [o + a * (i - t) / (r * r), e - a * o * e * (i - t), e + a * o * e * (i + t), 2 * Math.atan(Math.exp(t)) - va + a * (i - t) / r];
        var u = [1, 0, 0, 0, 0, 0, 0, 0, 0],
            s = [Math.sqrt(n), 0, 0, 0, 0, 0, 0, 0, 0],
            c = 0;
        for (r = Math.sqrt(1 - n), i = 1; Math.abs(s[c] / u[c]) > sa && 8 > c;) a = u[c++], s[c] = .5 * (a - r), u[c] = .5 * (a + r), r = M(a * r), i *= 2;
        e = i * u[c] * t;
    ...
    function en(t, n, a) {
        ...
        if (r) {
            ...
            return [on(Math.atan(1 / Math.sqrt(f)), a) * i(t), on(Math.atan(M((f / u - 1) / a)), 1 - a) * i(n)]
        }
        return [0, on(Math.atan(o), 1 - a) * i(n)]
    }

    function on(t, n) {
        if (!n) return t;
        if (1 === n) return Math.log(Math.tan(t / 2 + fa / 4));
        for (var a = 1, r = Math.sqrt(1 - n), e = Math.sqrt(n), o = 0; Math.abs(e) > sa; o++) {
            if (t % fa) {
                var i = Math.atan(r * Math.tan(t) / a);
                0 > i && (i += fa), t += i + ~~(t / fa) * fa
            } else t += t;
    ...
    function hn(t, n) {
        var a = (Math.SQRT2 - 1) / (Math.SQRT2 + 1),
            ...
            e = on(va, r * r),
            o = -1,
            i = Math.log(Math.tan(fa / 4 + Math.abs(n) / 2)),
            h = Math.exp(o * i) / Math.sqrt(a),
            u = un(h * Math.cos(o * t), h * Math.sin(o * t)),
            M = en(u[0], u[1], r * r);
    ...
    function un(t, n) {
        var a = t * t,
            r = n + 1,
            e = 1 - a - n * n;
        return [.5 * ((t >= 0 ? va : -va) - Math.atan2(e, 2 * t)), -.25 * Math.log(e * e + 4 * a) + .5 * Math.log(r * r + a)]
    }
    ...
    function sn(t) {
        function n(t, n) {
            ...
            var i = Math.sin(n),
                ...
                s = u(a * i + r * h * M),
                c = Math.sin(s),
                f = Math.abs(c) > sa ? s / c : 1;
            return [f * r * Math.sin(t), (Math.abs(t) > va ? f : -f) * (a * h - r * i * M)]
        }
        var a = Math.sin(t),
            r = Math.cos(t),
            e = cn(t);
        return e.invert = cn(-t), n.invert = function(t, n) {
            var r = Math.sqrt(t * t + n * n),
                o = -Math.sin(r),
                i = Math.cos(r),
                h = r * i,
                ...
                v = (r > va ? -1 : 1) * Math.atan2(t * o, r * Math.cos(f) * i + n * Math.sin(f) * o);
            return e.invert(v, f)
        }, n
    }
    ...
    function fn() {
        ...
        return a.parallel = function(r) {
            if (!arguments.length) return t / fa * 180;
            var e = a.rotate();
            return n(t = r * fa / 180).rotate(e)
        }, a.rotate = function(n) {
            return arguments.length ? (r.call(a, [n[0], n[1] - t / fa * 180]), o.origin([-n[0], -n[1]]), a) : (n = r.call(a), n[1] += t / fa * 180, n)
        }, a.stream = function(t) {
    ...
    function vn(t, n) {
        ...
        return arguments.length < 2 && (n = t), 1 === n ? ka : 1 / 0 === n ? gn : (a.invert = function(a, r) {
            var e = ka.invert(a / t, r);
            return e[0] *= n, e
        }, a)
    }
    ...
    function dn(t, n) {
        for (var a, r = Math.sin(n) * (0 > n ? 2.43763 : 2.67595), e = 0; 20 > e && (n -= a = (n + Math.sin(n) - r) / (1 + Math.cos(n)), !(Math.abs(a) < sa)); e++);
    ...
    function bn(t) {
        function n(n, s) {
            var c, f = Math.abs(s);
            if (f > r) {
                var v = Math.min(t - 1, Math.max(0, Math.floor((n + fa) / M)));
                n += fa * (t - 1) / t - v * M, c = d3.geo.collignon.raw(n, f), c[0] = c[0] * e / o - e * (t - 1) / (2 * t) + v * e / t, c[1] = i + 4 * (c[1] - h) * u / e, 0 > s && (c[1] = -c[1])
            } else c = a(n, s);
            return c[0] /= 2, c
        }
        var a = d3.geo.cylindricalEqualArea.raw(0),
            r = _a * fa / 180,
            e = 2 * fa,
            o = d3.geo.collignon.raw(fa, r)[0] - d3.geo.collignon.raw(-fa, r)[0],
            i = a(0, r)[1],
            h = d3.geo.collignon.raw(0, r)[1],
            u = d3.geo.collignon.raw(0, va)[1] - h,
            M = 2 * fa / t;
        return n.invert = function(n, r) {
            n *= 2;
            var s = Math.abs(r);
            if (s > i) {
                var c = Math.min(t - 1, Math.max(0, Math.floor((n + fa) / M)));
                n = (n + fa * (t - 1) / t - c * M) * o / e;
                var f = d3.geo.collignon.raw.invert(n, .25 * (s - i) * e / u + h);
                return f[0] -= fa * (t - 1) / t - c * M, 0 > r && (f[1] = -f[1]), f
            }
            return a.invert(n, r)
        }, n
    }
    ...
    function wn(t) {
        function n(n, e) {
            ...
            if (f && 2 > f) {
                ...
                } while (Math.abs(v) > ca && --g > 0);
                h = i * Math.sqrt(w), u = n * p / fa
            } else h = i * (t + f), u = n * o / fa;
            return [h * Math.sin(u), M - h * Math.cos(u)]
        }
        var a, r = 1 + t,
            e = Math.sin(1 / r),
            o = h(e),
            i = 2 * Math.sqrt(fa / (a = fa + 4 * o * r)),
        ...
        return n.invert = function(t, n) {
            ...
            return [h(t / Math.sqrt(e)) * fa / g, h(1 - 2 * (v - s * o - r * l + (1 + c - 2 * r * f) * g) / a)]
        }, n
    }
    ...
    function mn(t, n) {
        return n > -za ? (t = ya(t, n), t[1] += Ba, t) : E(t, n)
    }

    function yn(t, n) {
        return Math.abs(n) > za ? (t = ya(t, n), t[1] -= n > 0 ? Ba : -Ba, t) : E(t, n)
    }

    function Sn(t, n) {
        return [3 * t / (2 * fa) * Math.sqrt(fa * fa / 3 - n * n), n]
    }

    function Qn(t) {
        function n(n, a) {
            if (Math.abs(Math.abs(a) - va) < sa) return [0, 0 > a ? -2 : 2];
            var r = Math.sin(a),
        ...
        return n.invert = function(n, a) {
            var r = Math.abs(a);
            if (Math.abs(r - 2) < sa) return n ? null : [0, i(a) * va];
            if (r > 2) return null;
            n /= 2, a /= 2;
            var e = n * n,
                o = a * a,
    ...
    function Pn(t) {
        function n(n, e) {
            var o = e - t,
                i = Math.abs(o) < sa ? n * a : Math.abs(i = fa / 4 + e / 2) < sa || Math.abs(Math.abs(i) - va) < sa ? 0 : n * o / Math.log(Math.tan(i) / r);
            return [i, o]
        }
        var a = Math.cos(t),
            r = Math.tan(fa / 4 + t / 2);
        return n.invert = function(n, e) {
            var o, i = e + t;
            return [Math.abs(e) < sa ? n / a : Math.abs(o = fa / 4 + i / 2) < sa || Math.abs(Math.abs(o) - va) < sa ? 0 : n * Math.log(Math.tan(o) / r) / e, i]
        }, n
    }

    function kn(t, n) {
        return [t, 1.25 * Math.log(Math.tan(fa / 4 + .4 * n))]
    }
    ...
    function _n(t) {
        ...
        return n.invert = function(n, r) {
            ...
            } while (Math.abs(g) + Math.abs(d) > sa * sa && --e > 0);
            if (e) {
    ...
    function zn() {
        ...
        return a.coefficients = function(a) {
            return arguments.length ? n(t = "string" == typeof a ? Fa[a] : a) : t
        }, a
    }
    ...
    function Fn(t, n) {
        for (var a, r = (1 + Math.SQRT1_2) * Math.sin(n), e = n, o = 0; 25 > o && (e -= a = (Math.sin(e / 2) + Math.sin(e) - r) / (.5 * Math.cos(e / 2) + Math.cos(e)), !(Math.abs(a) < sa)); o++);
        return [t * (1 + 2 * Math.cos(e) / Math.cos(e / 2)) / (3 * Math.SQRT2), 2 * Math.sqrt(3) * Math.sin(e / 2) / Math.sqrt(2 + Math.SQRT2)]
    }

    function An(t, n) {
        for (var a, r = Math.sqrt(6 / (4 + fa)), e = (1 + fa / 4) * Math.sin(n), o = n / 2, i = 0; 25 > i && (o -= a = (o / 2 + Math.sin(o) - e) / (.5 + Math.cos(o)), !(Math.abs(a) < sa)); i++);
    ...
    function Dn(t) {
        ...
        var a = Math.sin(t);
        return n.invert = function(n, r) {
            if (Math.abs(r += t) < sa) return [a ? 2 * Math.atan(a * n / 2) / a : n, 0];
            var e, o = n * n + r * r,
                i = 0,
                M = 10;
            do {
            ...
            } while (Math.abs(e) > sa && --M > 0);
            var v = n * (s = Math.tan(i)),
                l = Math.tan(Math.abs(r) < Math.abs(i + 1 / s) ? .5 * h(v) : .5 * u(v) + fa / 4) / Math.sin(i);
            return [a ? 2 * Math.atan(a * l) / a : 2 * l, i]
        }, n
    }

    function Ln(t, n) {
        var a, r = Math.min(18, 36 * Math.abs(n) / fa),
            e = Math.floor(r),
            o = r - e,
            i = (a = Ga[e])[0],
            h = a[1],
            u = (a = Ga[++e])[0],
            M = a[1],
            s = (a = Ga[Math.min(19, ++e)])[0],
            c = a[1];
        return [t * (u + o * (s - i) / 2 + o * o * (s - 2 * u + i) / 2), (n > 0 ? va : -va) * (M + o * (c - h) / 2 + o * o * (c - 2 * M + h) / 2)]
    }
    ...
    function In() {
        ...
        return r.distance = function(r) {
            ...
            return arguments.length ? a(t, n = r * fa / 180) : 180 * n / fa
        }, r
    }
    ...
    function Nn() {
        ...
        return delete a.rotate, a.points = function(a) {
            ...
            var e = d3.geo.interpolate(a[0], a[1]),
                ...
                u = .5 * e.distance,
                M = (i[0] < 0 ? -1 : 1) * i[1] * ga,
                s = h(Math.sin(M) / Math.sin(u));
            return r.call(i, [-o[0], -o[1], -s * da]), n(2 * u)
        }, a
    }
    ...
    function Vn() {
        ...
        return delete a.rotate, a.points = function(a) {
            ...
            var e = d3.geo.interpolate(a[0], a[1]),
                ...
                M = (i[0] < 0 ? -1 : 1) * i[1] * ga,
                s = h(Math.sin(M) / Math.sin(u));
            return r.call(i, [-o[0], -o[1], -s * da]), n(u)
        }, a
    }
    ...
    function Wn(t, n) {
        ...
        var a = Math.abs(n / va),
            r = h(a);
        if (Math.abs(t) < sa || Math.abs(Math.abs(n) - va) < sa) return [0, i(n) * fa * Math.tan(r / 2)];
        var e = Math.cos(r),
            o = Math.abs(fa / t - t / fa) / 2,
            u = o * o,
            M = e / (a + e - 1),
            s = M * (2 / a - 1),
            c = s * s,
            ...
            v = M - c,
            l = u + M;
        return [i(t) * fa * (o * v + Math.sqrt(u * v * v - f * (M * M - c))) / f, i(n) * fa * (s * l - o * Math.sqrt((u + 1) * f - l * l)) / f]
    }
    ...
    function Xn(t, n) {
        ...
        return [i(t) * fa * s, i(n) * fa * M(1 - s * (2 * o + s))]
    }
    ...
    function Yn(t, n) {
        ...
        var a = n / va,
            r = h(a);
        if (Math.abs(t) < sa || Math.abs(Math.abs(n) - va) < sa) return [0, fa * Math.tan(r / 2)];
        var e = (fa / t - t / fa) / 2,
            o = a / (1 + Math.cos(r));
        return [fa * (i(t) * M(e * e + 1 - o * o) - e), fa * o]
    }

    function Zn(t, n) {
        if (!n) return [t, 0];
        var a = Math.abs(n);
        if (!t || a === va) return [0, n];
        var r = a / va,
            e = r * r,
            ...
            f = t / va,
            v = f + 1 / f,
            l = i(Math.abs(t) - va) * Math.sqrt(v * v - 4),
            g = l * l,
            d = s * (e + h * g - 1) + (1 - e) * (e * (c * c + 4 * h) + 12 * u * h + 4 * h * h),
            b = (l * (s + h - 1) + 2 * M(d)) / (4 * s + g);
        return [i(t) * va * b, i(n) * va * M(1 + l * Math.abs(b) - b * b)]
    }
    ...
    function aa(t, n) {
        var a = f(t, n);
        return [(a[0] + t / va) / 2, (a[1] + n) / 2]
    }
    d3.geo.project = function(t, a) {
        var r = a.stream;
        if (!r) throw new Error("not yet supported");
        return (t && ra.hasOwnProperty(t.type) ? ra[t.type] : n)(t, r)
    };
    var ra = {
            Feature: t,
        ...
        ha = {
            ...
            lineEnd: function() {
                ea.length && (oa.push(ea), ea = [])
            },
            result: function() {
                var t = oa.length ? oa.length < 2 ? {
                    type: "LineString",
                    coordinates: oa[0]
                } : {
        ...
        ua = {
            ...
            lineEnd: function() {
                var t = ea.length;
                if (t) {
                    do ea.push(ea[0].slice()); while (++t < 4);
                    oa.push(ea), ea = []
                }
            },
            polygonEnd: a,
        ...
        ga = fa / 180,
        da = 180 / fa,
        ba = d3.geo.projection,
        pa = d3.geo.projectionMutator;
    ...
    var ma = Q(fa),
        ya = R(Math.SQRT2 / va, Math.SQRT2, fa);
    ...
    (d3.geo.bromley = function() {
        ...
        do {
            var i = r / 2,
                ...
                E = x * R,
                P = x * T,
                k = (k = 2 * c + Math.SQRT2 * v * (M - u)) * k * q,
                _ = (Math.SQRT2 * M * c * v + f) / k,
                z = -(Math.SQRT2 * u * s) / (v * k),
                B = m * E - 2 * _ / q + d * (_ + _ / w),
                F = m * P - 2 * z / q + d * (z + z / w),
                A = l * y * E - 2 * R / (1 + g) + d * y * R + d * l * (_ - _ / w),
                G = l * y * P - 2 * T / (1 + g) + d * y * T + d * l * (z - z / w),
                j = F * A - G * B;
            if (!j) break;
            var C = (Q * F - S * G) / j,
                D = (S * A - Q * B) / j;
    ...
    mn.invert = function(t, n) {
        ...
        do {
            var o = a * a,
                ...
                c = .975534 - h * (.119161 + 3 * o * .0143059 + .0547009 * h),
                ...
                b = (M * v - s * c) / g;
            a -= d, r -= b
        } while ((Math.abs(d) > sa || Math.abs(b) > sa) && --e > 0);
        return e && [a, r]
        ...
        return ba(En)
    }).raw = En, (d3.geo.loximuthal = function() {
        return x(Pn).parallel(40)
    }).raw = Pn, kn.invert = function(t, n) {
        return [t, 2.5 * Math.atan(Math.exp(.8 * n)) - .625 * fa]
    ...
    (d3.geo.modifiedStereographic = zn).raw = _n, Bn.invert = function(t, n) {
        var a = Math.sqrt(6),
            r = Math.sqrt(7),
            e = 3 * h(n * r / 9);
        ...
        return ba(Fn)
    }).raw = Fn, An.invert = function(t, n) {
        var a = Math.sqrt(6 / (4 + fa)),
            r = n / a;
        return Math.abs(Math.abs(r) - va) < sa && (r = 0 > r ? -va : va), [1.5 * t / (a * (.5 + Math.cos(r))), h((r / 2 + Math.sin(r)) / (1 + fa / 4))]
    }, (d3.geo.mtFlatPolarSinusoidal = function() {
        return ba(An)
    }).raw = An, Gn.invert = function(t, n) {
        var a, r = n,
            e = 25;
        do {
            var o = r * r,
    ...
    var ja = function() {
        var t = 4 * fa + 3 * Math.sqrt(3),
            n = 2 * Math.sqrt(2 * fa * Math.sqrt(3) / t);
