function szDBG(txt){/* console.log(txt);*/} //sz2do once you click once, the hover and see country name facility is lost. Update szDataDomain,szYDataDomain?
var szFooterText="Source: The Economist Intelligence Unit"; var szMapIDname="democracyMap",szCurrentYear=2017;
var szHues=['#d73027', '#f46d43', '#fdae61', '#fee090', '#ffffbf', '#e0f3f8', '#abd9e9', '#74add1', '#4575b4']; 
var szHueScale=[2, 3, 4, 5, 6, 7, 8, 9, 10]; var arrKMBX =[5, 161, 297, 433];
...
var strHighlightColourStrong = "#9db4bf";var intSelBut = -1;var szSelectedItem = "";var szBegYearCounter = szYrsOfData.length;
...
var szD3HeatMap="heatMapSVG",szD3HeatMapHeight=552;var szKeyNumHues=[0, 2, 3, 4, 4, 5, 6, 6, 7, 8, 8, 9, 10];
var szIniMapFillhue="#111111",szIniMapStroke="#888";// szIniMapFillhue="#d9dbd5",szIniMapStroke="#fff";
var arrKey=["Authoritarian regime", "Hybrid regime", "Flawed democracy", "Full democracy"];var arrKeyShort=["AUTH_REG", "HYBR_REG", "FLAW_DEM", "FULL_DEM"];
...
var szGridRow="blockRow",szInitBlockWidth=545;var szCssFootNote="sz2FootNote"
...
function s2MapHues(szBegYear) {intDuration = 0;if (intSelBut == -1) {intDuration = 800 }
  szLocalData.forEach(function(d) {szBegYearPos = szYrsOfData.indexOf(szBegYear)
    d3.select("#" + d.Code) .attr("class", function() {
      for (c=0; c < szHues.length; c++) {if (d.DI[szBegYearPos] > 8) {return "szNations FULL_DEM";}
        if (d.DI[szBegYearPos] > 6) {return "szNations FLAW_DEM";}; if (d.DI[szBegYearPos] > 4) {return "szNations HYBR_REG";}; return "szNations AUTH_REG"; }})  
            .transition().duration(intDuration)  .attr("fill", function() {	for (c = 0; c < szHues.length; c++) {if (d.DI[szBegYearPos] < szHueScale[c]) {return szHues[c]	} }  })  })  
}
function szAddHeatMap(szHues) {var intBlockWidth=szInitBlockWidth / szYrsOfData.length; var intBlockHeight=3; var intBlockYOffset=38; var intBlockXOffset=10;//  szDBG("szHues.length="+szHues.length)
  ...
  d3.selectAll("."+szGridRow) .selectAll(".blockCol").data(d3.range(szYrsOfData.length)) .enter() .append("rect") .attr("stroke-width", 0)
    ...
    .attr("class", function(d, i) {return "allBlocks b_" + arrDIPos[d3.select(this)[0][0].parentElement.id.replace("g_r_", "")][i] + " c_" + i;    })
  ...
  d3.select("#"+szD3HeatMap).selectAll(".blockVals") .data(d3.range(szYrsOfData.length)).enter() .append("text") .attr("x", function(d, i) {return (intBlockWidth / 1.5 + (i * intBlockWidth)) })
    .attr("text-anchor", "middle").attr("class", "blockVals") .attr("id", function(d, i) {return "bv_" + i; }) }
function highlightCountry() {if (szSelectedItem==="") { d3.selectAll(".allBlocks")  .attr("height", 3)  .attr("opacity", 1);
   d3.select("#"+szSelFromList).selectAll("option")[0][0].selected = true; s2SetSelListBkgndHue(); d3.selectAll(".blockVals")  .text("") } 
else {s2SetSelListBkgndHue(); countryVals = szLocalData[s2GetArrayPos(szSelectedItem)].DI;  d3.selectAll(".allBlocks") .attr("height", 3) .attr("opacity", 0.3);
        d3.selectAll(".blockVals") .text(function(d, i) { return numFormat(countryVals[i]);  }) .attr("opacity", 0) /* .transition()   .duration(105) */         .attr("opacity", 1)
            .attr("y", function(d, i) {return +d3.select("#b_"+ szSelectedItem +"_" + i)[0][0].parentNode.getAttribute("transform").split(",")[1].replace(")", "") + 13;   })
            .attr("fill", function(d, i) { var intBaseColour = szHues.indexOf(d3.select("#b_"+ szSelectedItem +"_" + i).attr("fill"));
                if (intBaseColour > 5 || intBaseColour < 3) {return "#fff"; }; return "#000"; })
...
function szToggleMapTabKey() {d3.selectAll("."+szCssTabBtns).selectAll("rect").attr("fill", "#fff"); d3.selectAll("."+szCssTabBtns).selectAll("text").attr("fill", "#000"); d3.selectAll(".szNations").attr("fill-opacity", 1);
    if (intSelBut > -1) {d3.selectAll("."+szCssTabBtns).selectAll("rect")[intSelBut][0].setAttribute("fill", strHighlightColourStrong);d3.selectAll("."+szCssTabBtns).selectAll("text")[intSelBut][0].setAttribute("fill", "#fff")
        d3.selectAll(".szNations").attr("fill-opacity", 0.1); d3.selectAll("." + arrKeyShort[intSelBut]).attr("fill-opacity", 1); }}
...
function s2SetSelListBkgndHue() {if (szSelectedItem === "") {szNewSelListBkgrndHue="#111"} else {szNewSelListBkgrndHue=d3.select("#b_" + szSelectedItem + "_" + szYrsOfData.indexOf(szBegYear)).attr("fill");
   ...
   var intBaseColour = szHues.indexOf(szNewSelListBkgrndHue);  if (intBaseColour > 5 || intBaseColour < 3) {szNewSelListItemHue="#fff";} }
